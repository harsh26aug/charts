...
import { getNiftyStockHistory } from '../../services/stocks';
import type { DateRange, NiftyStockRecord, StockPagination } from '../../types/stock';
import {
  formatDiff,
  formatPct,
  formatPrice,
...
interface NiftyTableProps {
  dateRange: DateRange;
}

export const NiftyTable = ({ dateRange }: NiftyTableProps) => {
  const [isLoading, setIsLoading] = useState(false);
  const [data, setData] = useState<NiftyStockRecord[]>([]);
  ...
  const fetchData = async (page: number, limit: number, range: DateRange) => {
    setIsLoading(true);
    try {
      const res = await getNiftyStockHistory({
        page,
        limit,
        ...toDateParams(range),
      });
  ...
  const columns: ColumnsType<NiftyStockRecord> = [
    {
      title: 'Date',
      dataIndex: 'tradeDate',
      key: 'tradeDate',
    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          <div className={`cell-main ${getDiffClass(record.diff.closeDiff)}`}>
            {formatPrice(record.price.close)}
          </div>
          <div className={`cell-diff ${getDiffClass(record.diff.closeDiff)}`}>
            {formatDiff(record.diff.closeDiff)}
            <span className="pct">&nbsp;({formatPct(record.diff.closeDiffPct)})</span>
          </div>
        </>
      ),
    },
    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          <div className="cell-main">{formatQuantity(record.price.sharesTraded)}</div>
          <div className={`cell-diff ${getDiffClass(record.diff.sharesTradedDiff)}`}>
            {formatDiff(record.diff.sharesTradedDiff)}
            <span className="pct">&nbsp;({formatPct(record.diff.sharesTradedDiffPct)})</span>
          </div>
        </>
      ),
    },
    {
      ...
      render: (_, record) => (
        <>
          <div className="cell-main">{formatPrice(record.price.turnoverCr)}</div>
          <div className={`cell-diff ${getDiffClass(record.diff.turnoverCrDiff)}`}>
            {formatDiff(record.diff.turnoverCrDiff)}
            <span className="pct">&nbsp;({formatPct(record.diff.turnoverCrDiffPct)})</span>
          </div>
        </>
      ),
    },
